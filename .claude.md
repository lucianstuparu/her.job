...
curl -s -X POST \
  ...
  https://api.github.com/repos/lucianstuparu/her.job/issues \
...
curl -s -X PATCH \
  -H "Authorization: token $GH_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  https://api.github.com/repos/lucianstuparu/her.job/issues/ISSUE_NUMBER \
...
curl -s -H "Authorization: token $GH_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  "https://api.github.com/repos/lucianstuparu/her.job/issues?state=open&per_page=100"
```
...
**Project:** "Personal Knowledge System" (Project #2)
- **Project ID:** `PVT_kwHOAKRZo84BOBlm`
- **URL:** https://github.com/users/lucianstuparu/projects/2

### Custom Fields
- **Status** (Single Select):
  - Todo (ID: `f75ad846`) - Default for new issues
  - Next (ID: `34c960b0`)
  - In Progress (ID: `47fc9ee4`)
  - Done (ID: `98236657`)
  - Field ID: `PVTSSF_lAHOAKRZo84BOBlmzg82QFE`

- **Deadline** (Date):
  - Field ID: `PVTF_lAHOAKRZo84BOBlmzg829XM`

### Query Project Items
```bash
curl -s -X POST -H "Authorization: bearer $GH_TOKEN" -H "Content-Type: application/json" \
  -d '{"query":"query { node(id: \"PVT_kwHOAKRZo84BOBlm\") { ... on ProjectV2 { items(first: 50) { nodes { id content { ... on Issue { number title } } } } } } }"}' \
...
curl -s -X POST -H "Authorization: bearer $GH_TOKEN" -H "Content-Type: application/json" \
  -d '{"query":"query { node(id: \"PVT_kwHOAKRZo84BOBlm\") { ... on ProjectV2 { items(last: 10) { nodes { id content { ... on Issue { number title } } } } } } }"}' \
  https://api.github.com/graphql
```
...
ADD_RESPONSE=$(curl -s -X POST -H "Authorization: bearer $GH_TOKEN" -H "Content-Type: application/json" \
  -d "{\"query\":\"mutation { addProjectV2ItemById(input: {projectId: \\\"PVT_kwHOAKRZo84BOBlm\\\", contentId: \\\"$ISSUE_NODE_ID\\\"}) { item { id } } }\"}" \
  https://api.github.com/graphql)
...
# 3. Set the deadline
curl -s -X POST -H "Authorization: bearer $GH_TOKEN" -H "Content-Type: application/json" \
  -d "{\"query\":\"mutation { updateProjectV2ItemFieldValue(input: {projectId: \\\"PVT_kwHOAKRZo84BOBlm\\\", itemId: \\\"$ITEM_ID\\\", fieldId: \\\"PVTF_lAHOAKRZo84BOBlmzg829XM\\\", value: {date: \\\"2026-02-10\\\"}}) { projectV2Item { id } } }\"}" \
  https://api.github.com/graphql
...
ITEM_ID=$(curl -s -X POST -H "Authorization: bearer $GH_TOKEN" -H "Content-Type: application/json" \
  -d '{"query":"query { node(id: \"PVT_kwHOAKRZo84BOBlm\") { ... on ProjectV2 { items(last: 10) { nodes { id content { ... on Issue { number } } } } } } }"}' \
  https://api.github.com/graphql | grep -o 'PVTI_[^"]*","content":{"number":25' | sed 's/","content.*//')

# 2. Update status to "Next"
curl -s -X POST -H "Authorization: bearer $GH_TOKEN" -H "Content-Type: application/json" \
  -d "{\"query\":\"mutation { updateProjectV2ItemFieldValue(input: {projectId: \\\"PVT_kwHOAKRZo84BOBlm\\\", itemId: \\\"$ITEM_ID\\\", fieldId: \\\"PVTSSF_lAHOAKRZo84BOBlmzg82QFE\\\", value: {singleSelectOptionId: \\\"34c960b0\\\"}}) { projectV2Item { id } } }\"}" \
  https://api.github.com/graphql
```
